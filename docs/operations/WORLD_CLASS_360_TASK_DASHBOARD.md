...
| W360-003 | Xico City repo stabilization | Xico platform team | `C:/Users/Ultimate Warrior/Documents/GitHub/xico-city` | none | ⚠️ PARTIALLY UNBLOCKED 2026-04-30 — vitest peer conflict **FIXED**: updated `package.json` vitest `^1.6.0` → `^2.1.0` + @vitest/coverage-v8 `^1.6.0` → `^2.1.0` to resolve @cloudflare/vitest-pool-workers incompatibility (committed 00a8f82). Remaining blocker: `npm install` fails with `E401 Unauthorized` on `@adrper79-dot/*` packages (GitHub Packages auth token not configured in Xico repo `.npmrc` — environment-specific issue, not code defect). Once operator sets `.npmrc` token: `npm install` will generate package-lock.json and unblock W360-004. Scripts `lint` and `build` already exist in package.json (no action needed) |
| W360-004 | Xico City Worker health deploy | Xico platform team | Xico `wrangler.jsonc`, deploy workflow, service registry | W360-003 | ⚠️ BLOCKED 2026-04-29 — upstream W360-003 is blocked (missing lockfile + dependency/script issues in external `xico-city` repo), so deploy/health verification cannot proceed in this workspace |
| W360-005 | Practitioner Studio entitlement bridge | Revenue API team | product API, entitlement tables, Stripe webhook, credit ledger | video pipeline baseline | ⚡ IN PROGRESS (code complete, runtime gate pending) 2026-04-30 — all in-repo code passes gates: `packages/neon` lint ✓ typecheck ✓ 63/63 tests ✓, `packages/schedule` lint ✓ typecheck ✓ 50/50 tests ✓ after strict-test lint cleanup, plus `apps/schedule-worker` now includes `/stripe/health` ingress probe with local gates green (`lint`, `typecheck`, `test` 8/8). Runtime gate remains open: deployed `curl -i https://schedule-worker.adrper79.workers.dev/stripe/health` still returns `404`; `curl -i https://admin-studio-staging.adrper79.workers.dev/health` returns `200` (staging live), but production ingress checks (`admin-studio-production` health and webhook URL probe) still return `404`. Next operator step: run `.github/workflows/deploy-admin-studio.yml` with `workflow_dispatch env=production`, then re-run curl verification and Stripe test-card webhook ack check |
...
| W360-022 | User-journey SLOs | Observability team | docs/runbooks/slo, synthetic monitor | W360-007, W360-014 | ⚡ IN PROGRESS 2026-04-30 — `docs/operations/USER_JOURNEY_SLOS.md` defines 9 journeys (J01–J09) with SLO targets (99.9–99.5%), burn-rate thresholds, and p50/p95/error-rate budgets. Active probes in `apps/synthetic-monitor` (15/15 tests passing after J08 config update 2026-04-30): 4 worker health checks, 3 manifest integrity checks, 4 journey proxies (J01-J04) + **NEW J08 webhook ingress probe configured and ready**. J01-J04 live (schedule-worker, video-cron, admin-studio-staging, prime-self health endpoints all 200 OK). J08 configuration (`slo.journey.webhook` → `https://schedule-worker.adrper79.workers.dev/stripe/health`) added to TARGETS_JSON 2026-04-30 and validated locally (15/15 tests pass); will activate automatically once W360-005 production deployment completes (`/stripe/health` returns 200 in production). J05-J09 remaining probes (checkout, first-render, booking, dashboard) deployment-gated on W360-005/007/008/014 completion; SLO doc updated with deployment checklist in section 5 |
