...
| W360-022 | User-journey SLOs | Observability team | docs/runbooks/slo, synthetic monitor | W360-007, W360-014 | ⚡ IN PROGRESS 2026-04-30 — J08 ingress endpoint is now deployed and externally verified (`curl -i https://schedule-worker.adrper79.workers.dev/stripe/health` → `200`) and schedule-worker manifest includes `/stripe/health`. Synthetic monitor production deploy succeeded (run `25168329699`) and includes `slo.journey.webhook` target, but `/checks/run` currently reports false `404` for workers.dev targets while direct external curls return `200` (schedule-worker/video-cron/admin-studio-staging health checks). This is now tracked as an observability-runtime anomaly in monitor execution context; W360-022 remains open for this monitor fidelity fix plus remaining dependency-gated probes J05/J06/J07/J09. |
