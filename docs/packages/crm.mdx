---
title: "@latimer-woods-tech/crm"
description: Lead tracking, conversion events, and customer health view.
---

# @latimer-woods-tech/crm

CRM primitives: lead upsert, subscription conversion tracking, and a customer health view with churn risk scoring. Depends on `@latimer-woods-tech/neon` and `@latimer-woods-tech/analytics`.

## Installation

```bash
npm install @latimer-woods-tech/crm
```

## Database

```typescript
import { CREATE_CRM_LEADS_TABLE } from '@latimer-woods-tech/crm';
...
Updates a lead's status and MRR. Optionally fires a `subscription.converted` event to `@latimer-woods-tech/analytics`.
...
```typescript
import { trackLead, trackConversion, getCustomerView } from '@latimer-woods-tech/crm';
