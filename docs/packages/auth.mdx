...
Self-managed JWTs using the Web Crypto API (HMAC SHA-256). No external JWT library. Depends on `@factory/errors` and `@factory/logger`.

## Installation

```bash
npm install @factory/auth
```
...
```typescript
import { issueToken, jwtMiddleware, requireRole } from '@factory/auth';
