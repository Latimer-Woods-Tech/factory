...
TCPA suppression list management, FDCPA 24-hour contact window enforcement, and immutable consent records. Depends on `@factory/neon`.

## Installation

```bash
npm install @factory/compliance
```
...
```typescript
import { checkTCPA, logConsent, checkFDCPA, recordContact } from '@factory/compliance';
