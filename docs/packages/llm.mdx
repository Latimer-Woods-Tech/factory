...
LLM completions with a three-provider failover chain (Anthropic → Grok → Groq). Includes streaming and system-prompt binding. Depends on `@latimer-woods-tech/errors` and `@latimer-woods-tech/logger`.
...
Each failover is logged via `@latimer-woods-tech/logger`.
